...
PERIDIO_DEFCONFIG_DIR=$ABS_DEFCONFIG_DIR
...
if [[ -z $BUILDROOT_DL_DIR ]]; then
    if [[ -e $HOME/dl ]]; then
        ...
        BUILDROOT_DL_DIR=$(readlink_f "$BUILD_DIR/../dl")
...
create_buildroot_dir() {
    ...
    "$BASE_DIR/download-buildroot.sh" $BUILDROOT_VERSION $BUILDROOT_DL_DIR $OUTPUT_DIR
    ...
    if ! [[ -z $BUILDROOT_DL_DIR ]]; then
        ...
        ln -sf $BUILDROOT_DL_DIR $BUILDROOT_DIR/dl
    fi
